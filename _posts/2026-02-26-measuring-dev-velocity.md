...
## The Data: Story Points Per Sprint
...
(function() {
  ...
  const points = [3,5,2,0,0,0,21,5,5,0,0,0,13,0,0,0,0,0,10,2,0,3,8,16,21,3,13,0,0,0,0,13,5,5,0,8,0,8,7,17,9,0,20,8,13,0,8,0,0,0,0,8,5,13,0,8,10,3,28,0,0,8,0,41,0,0,13,8,0,5,5,2,0,0,14,8,0];
  const sma = [null,null,null,2.5,1.8,0.5,5.2,6.5,7.8,7.8,2.5,1.2,3.2,3.2,3.2,3.2,0.0,0.0,2.5,3.0,3.0,3.8,3.2,6.8,12.0,12.0,13.2,9.2,4.0,3.2,0.0,3.2,4.5,5.8,5.8,4.5,3.2,4.0,5.8,8.0,10.2,8.2,11.5,9.2,10.2,10.2,7.2,5.2,2.0,2.0,0.0,2.0,3.2,6.5,6.5,6.5,7.8,5.2,12.2,10.2,7.8,9.0,2.0,12.2,12.2,10.2,13.5,5.2,5.2,6.5,4.5,3.0,3.0,1.8,4.0,5.5,5.5];
  ...
  new Chart(document.getElementById('pointsChart'), {
    ...
    options: {
      ...
      scales: {
        y: { 
          beginAtZero: true, 
          title: { display: true, text: 'Story Points', color: '#999' },
      ...
      plugins: {
        ...
        annotation: {
          annotations: {
            summer23: { type: 'box', xMin: 9, xMax: 11, backgroundColor: 'rgba(100,100,255,0.08)', label: { display: true, content: '🏖️', position: 'start' }},
            xmas23: { type: 'box', xMin: 20, xMax: 21, backgroundColor: 'rgba(100,100,255,0.08)', label: { display: true, content: '🎄', position: 'start' }},
            summer24: { type: 'box', xMin: 35, xMax: 37, backgroundColor: 'rgba(100,100,255,0.08)', label: { display: true, content: '🏖️', position: 'start' }},
            xmas24: { type: 'box', xMin: 46, xMax: 47, backgroundColor: 'rgba(100,100,255,0.08)', label: { display: true, content: '🎄', position: 'start' }},
            summer25: { type: 'box', xMin: 61, xMax: 63, backgroundColor: 'rgba(100,100,255,0.08)', label: { display: true, content: '🏖️', position: 'start' }},
            xmas25: { type: 'box', xMin: 72, xMax: 73, backgroundColor: 'rgba(100,100,255,0.08)', label: { display: true, content: '🎄', position: 'start' }},
            aiExplore: { type: 'line', xMin: 64, xMax: 64, borderColor: 'rgba(255,180,50,0.5)', borderWidth: 2, borderDash: [6,4], label: { display: true, content: 'AI Experimenting', color: '#ffb432', backgroundColor: 'transparent', position: 'start', font: {size: 10} }}, aiSerious: { type: 'line', xMin: 73, xMax: 73, borderColor: 'rgba(255,80,80,0.7)', borderWidth: 2, borderDash: [6,4], label: { display: true, content: 'AI Serious', color: '#ff5050', backgroundColor: 'transparent', position: 'start' }}
          }
        }
      }
    }
...
You can see the natural pattern: ramp-up after joining in 2023 as I learned the codebase, a steady rhythm through 2024, and then things start shifting in late 2025 as AI tools entered the workflow.
...
## The Data: Tickets Closed Per Sprint
...
(function() {
  const labels = ["Mar 23","","Apr 23","","May 23","","Jun 23","","Jul 23","","","Aug 23","Sep 23","","","Oct 23","","Nov 23","","Dec 23","","Jan 24","","","Feb 24","","Mar 24","","Apr 24","","","May 24","Jun 24","","Jul 24","","","Aug 24","","Sep 24","","Oct 24","","Nov 24","","Dec 24","","","Jan 25","","Feb 25","","Mar 25","","Apr 25","","May 25","","Jun 25","","","Jul 25","","Aug 25","","Sep 25","","Oct 25","","Nov 25","","Dec 25","","Jan 26","","Feb 26"];
  const tickets = [1,1,2,2,1,0,2,5,2,3,1,1,1,2,1,2,1,0,4,2,0,1,1,2,6,1,4,2,0,2,0,2,4,2,2,2,1,2,5,6,6,0,1,1,2,0,1,3,8,2,1,3,5,4,2,1,2,2,2,5,1,5,0,3,1,2,3,3,4,5,7,2,0,1,5,3,0];
  const sma = [null,null,null,1.5,1.5,1.2,1.2,2.0,2.2,3.0,2.8,1.8,1.5,1.2,1.2,1.5,1.5,1.0,1.8,1.8,1.5,1.8,1.0,1.0,2.5,2.5,3.2,3.2,1.8,2.0,1.0,1.0,2.0,2.0,2.5,2.5,1.8,1.8,2.5,3.5,4.8,4.2,3.2,2.0,1.0,1.0,1.0,1.5,3.0,3.5,3.5,3.5,2.8,3.2,3.5,3.0,2.2,1.8,1.8,2.8,2.5,3.2,2.8,2.2,2.2,1.5,2.2,2.2,3.0,3.8,4.8,4.5,3.5,2.5,2.0,2.2,2.2];
  ...
  new Chart(document.getElementById('ticketsChart'), {
    ...
    options: {
      ...
      scales: {
        y: { 
          beginAtZero: true, 
          title: { display: true, text: 'Tickets Closed', color: '#999' },
          grid: { color: 'rgba(255,255,255,0.1)' },
          ticks: { color: '#999' }
        },
        x: { 
...
1. **The August 2025 spike** - 41 story points in a single sprint. That wasn't AI, just a really good month (maybe I helped someone substantially on their ticket). It's an outlier, not a trend.
...
4. **Bugs don't have points** - 108 of my 175 tickets were unpointed bugs. The story points chart only shows part of the picture.

The shape you'd expect is there: a ramp from March 2023 as I learned the codebase, levelling off through mid-2024, then a steady baseline until late 2025. Now we watch for what happens next.
